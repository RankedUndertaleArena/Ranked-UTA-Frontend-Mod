...
public class QueueCommand implements ICommand {
    ...
    private static class ExecuteQueue implements Command<ServerCommandSource> {
        private final String mode;

        public ExecuteQueue(String mode) {
            this.mode = mode;
        }
                ...
                switch (statusCode) {
                    case OK -> {
                        ...
                        for (int i = 0; i < members.length(); i++) {
                            ...
                            else member.sendMessage(Text.literal("You joined " + mode + " queue...").setStyle(Style.EMPTY.withColor(0x55FF55)));
                        }
                        return 1;
                    }
